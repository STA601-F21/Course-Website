...
```{r rbetasamp}
W <- rbeta(500, shape1 = 1/2, shape2 = 1/2)
```
...
x[1] <- xprev
for(i in 1:549){
  x[i+1] <- 0.8*xprev + rt(1, df = 5)
  xprev <- x[i+1]
}
#
plot(1:length(x), x, type = 'l', xlab = "Time", ylab = "x")
points(which(x == min(x)), min(x), col = 'red')
points(which(x == max(x)), max(x), col = 'blue')
