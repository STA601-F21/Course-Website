---
title: "STA 602L Lab 10: Metropolis-Hastings"
...
+ Generate $x^*$ from $J_s\left( x^* | ~x^{(s)}\right)$
+ Compute the acceptance ratio $r = \frac{p_0\left(x^*\right)}{p_0\left(x^{(s)}\right)} \times \frac{J_s\left( x^{(s)} |~ x^*\right)}{J_s\left( x^* | ~x^{(s)}\right)}$
...
1. Suppose you specify a normal prior for $\boldsymbol{\beta}$. Try to find the full conditional density function $p(\boldsymbol{\beta} | Y)$. Can you sample from it directly? You don't need to simplify completely.
...
$$
\boldsymbol{\beta}_k^* | \boldsymbol{\beta}_k^{(s)} \sim N(0, \delta^2 I),
$$
with $\delta = 0.05$.
