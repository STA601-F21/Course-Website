...
- The .hlight[probability mass function (pmf)] of a random variable $Y$, $p(y)$ describes the probability associated with each possible value of $Y$.
...
- **Example**: $Y =$ number of in individuals with type I diabetes out of sample of $n$ surveyed.
...
.block[
.small[
$$f(y) = \frac{1}{B(a,b)} \theta^{a-1} (1-\theta)^{b-1}; \ \ \ y  \in (0,1), \ a > 0, \ b > 0.$$
]
]
