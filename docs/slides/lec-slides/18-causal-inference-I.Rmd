...
- .hlight[Multiple versions of treatment]: Sometimes the treatment $W$ doesn't have a clear meaning, as it has many versions.
...
- The key identifying assumptions are on the assignment mechanism
...
.small[
$$
\mathbb{Pr}(Y(w)) = \mathbb{Pr}[Y^\textrm{obs}|W=w], \ \ \ \ w= 0,1.
$$
]
]
...
.small[
$$
\textrm{ATE} = \mathbb{E}[Y^\textrm{obs}|W=1] - \mathbb{E}[Y^\textrm{obs}|W=0].
$$
]
]
...
& = \mathbb{E}[Y^\textrm{obs}|W=1] - \mathbb{E}[Y^\textrm{obs}|W=0],
\end{split}
$$
]
]
