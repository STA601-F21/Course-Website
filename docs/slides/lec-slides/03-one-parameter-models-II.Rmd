...
- Let's go back to Bernoulli data. Suppose $y_i,\ldots,y_n \overset{iid}{\sim} \textrm{Bernoulli}(\theta)$.
...
- Suppose $y_i,\ldots,y_n \overset{iid}{\sim} \textrm{Po}(\theta)$.
  <div class="question">
...
.block[
.small[
$$\mathbb{E}[\theta] = \dfrac{a}{b}; \ \ \mathbb{V}[\theta] = \dfrac{a}{b^2}.$$
]
]
...
- Generally, it turns out that if 
  + $f(y_i; \theta): y_i,\ldots,y_n \overset{iid}{\sim} \textrm{Po}(\theta)$, and
...
- This is the .hlight[Poisson-Gamma model]. We will dive deeper with examples next time.
