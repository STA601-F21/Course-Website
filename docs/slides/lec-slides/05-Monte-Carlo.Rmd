...
- Thus, we have one single observation with $x_i = 2$ for this city.
...
- Suppose reviews of asthma mortality rates around the world suggest rates above 1.5 per 100,000 are rare in Western countries, with typical rates around 0.6 per 100,000.
...
- A few options here, but let's go with $\textrm{Ga}(3,5)$.
...
- In fact, $\Pr[\theta \geq 2 | x,y] = 0.99$, so that there is very little chance that we see more than 2 asthma-related deaths per 100,000 people in this city.
...
Posterior is to the right of the prior since the data suggests higher mortality rates are more likely than the prior suggests.
...
- <div class="question">
Can we figure out the family of distributions that would be conjugate for this density?
</div>

--

- Let's explore this using the .hlight[exponential distribution]. The exponential distribution is often used to model "waiting times" or other random variables $(\in (0,\infty) )$ often measured on a time scale.
...
$$f(y) = \theta e^{-y\theta}; \ \ \ y > 0.$$
  ...
  where $\theta$, the .hlight[rate parameter], and $\mathbb{E}[y] = 1/\theta$.
...
- While we will focus on use Monte Carlo integration for Bayesian inference, the development is general and applies to any pdf/pmf $p(\theta)$.
...
- .hlight[Importance sampling] is one of the first steps into Monte Carlo analysis, in which simulated values from one distribution are used to explore another.

--

- Simulation from the "wrong distribution" can be incredibly useful as we will see later in this course.
...
- Setup:
  + $p(\theta)$ is the density we are interested in sampling from;
  
  + $p(\theta)$ is tough to sample from but we are able to evaluate $p(\theta)$) as a function at any point; and
  
  + $g(\theta)$ is an .hlight[importance sampling distribution] that is easier to sample from.
...
- Usually, the context is one in which $g(\theta)$ has been derived as an analytic approximation to $p(\theta)$); and the closer the approximation, the more accurate the resulting importance sampling MC analysis will be.
...
- The measure "how wrong" at each simulated $\theta_m$ value is the .hlight[importance weight]
.block[
.small[
...
- Clearly, the closer $g$ is to $p$, the better the results.
...
- .hlight[Rejection sampling] utilizes the same importance ratio ideas, but leads to exact corrections and so exact samples from $p(\theta)$.

--

- Procedure:

  1. Let $p(\theta)$, $g(\theta)$ and $w(\theta)$ all be defined as before.
...
- Suppose we want to sample from $f_{[a,b]}(\theta)$, that is, a known standard pdf $f(\theta)$ truncated to $[a,b]$.
...
plot(p,f1,type='l',col='green4',xlim=c(0,1),ylab='Density', xlab=expression(theta),
     ylim=c(0,6),lty=1.5)
lines(density(THETA),type='l',col='red4',lty=1.5)
labels <- c("True Density", "Samples (Rej. Samp.)")
legend("topright", inset=.05, labels, lwd=2, lty=1.5, col=c('green4','red4'))
```
